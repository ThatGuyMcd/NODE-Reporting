...
import { VATRate, VAT_RATES, InvoiceItem } from '@/types';
import { generateQuoteNumber, generateId, calculateVAT, getVatRateLabel, getVatRateValue } from '@/utils/helpers';
import DatePickerInput from '@/components/DatePickerInput';
import Colors from '@/constants/colors';

export default function CreateQuoteScreen() {
  const router = useRouter();
  const { addQuote, searchProducts } = useBusiness();
  ...
  const handleSave = () => {
    ...
    if (total <= 0) {
      Alert.alert('Error', 'Quote total must be greater than zero');
      return;
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <View style={styles.section} testID="quote-items-section">
        ...
        <View style={styles.section}>
          ...
          <View style={styles.inputGroup}>
            ...
            <TextInput
              style={[styles.input, styles.textArea]}
              placeholder="Terms, conditions, additional details..."
              placeholderTextColor={Colors.textMuted}
              multiline
              numberOfLines={3}
              value={notes}
